...
Die Erde ist aus heutiger Sicht der einzige Planet der Leben aufweist. Trotz allen Einwirkungen über vier Milliarden Jahren sind wir in der glücklichen Lage gute Konditionen vorzufinden. Dazu gehören für uns Menschen und die meisten Lebewesen vor allem Wasser, Sauerstoff und ein mildes Klima.
...
\end{quote}
Es könnte also sein, dass auf den meisten Planeten im Sonnensystem einmal gute Konditionen für leben bestanden. Aber nur die Erde konnte mit einem anhaltenden, lebensfreundlichen Klima das Leben aufrecht erhalten. Tabelle \ref{planeten_comparison} zeigt ein Klima-Vergleich der vier sonnennächsten Planeten.
\begin{center}
\begin{table}
	\center
...
\caption{Klimas der Planeten im Vergleich}
\label{planeten_comparison}
\end{table}
\end{center}
Flüssiges Wasser und gemässigte Temperaturen sind nur noch auf der Erde in grossen Mengen auffindbar.
%Andere Planeten andere entwicklung
Alle anderen Planeten konnten offenbar kein freundliches Klima bewahren.
Die Wassermassen gefroren unter der Oberfläche oder entwichen aus der Atmosphäre ins All. Das letztere ist besonders schlimm, da der Vorgang irreversibel ist. Der Planet verliert somit seine lebensfreundlichen Lage für immer.

Doch wie kommen diese extremen Konditionen zu Stande? Man könnte meinen, dass primär die Distanz der Planeten zur Sonne für deren Temperatur ausschlaggebend ist. Jedoch nimmt diese über die Planeten mit der Entfernung zur Sonne nicht stetig ab.
...
In diesem Kapitel gehe ich der Frage nach, ob Venus und Mars alleine aufgrund ihrer orbitalen Eigenschaften nicht fähig waren, ein lebensfreundliches Klima zu behalten. Die Nullhypothese lautet somit, dass die genannten Planeten \textit{nicht} durch ihre Umlaufbahn oder Grösse benachteiligt sind, um Lebensfreundliche Klimabedingungen zu halten.
...
Mit der Annahme, dass grosse Mengen an Wasser auf allen Planeten vorhanden war, besteht auf allen Planeten die gleichen Voraussetzungen für mögliches Leben und ein Klima, an das wir uns auf der Erde gewohnt sind.
%TODO "Universalplanet" vewenden?
...
Auf Grund der zum Teil starken Vereinfachungen müssen einzelne freie Parameter, gekennzeichnet mit $\xi$'s, abgeschätzt werden. Durch diese kann das System getrimmt werden um überhaupt plausible Lösungen zu erreichen.
...
Das gleiche Modell wird für die Planeten Merkur, Venus, Erde und Mars Simuliert, wobei lediglich deren Mittlere Distanz $a_\text{planet}$ zur Sonne und Radius $R_\text{planet}$ eingesetzt werden.
...
\end{equation}
aus der Entfernung $a_{\text{planet}}$ des Planeten zur Sonne berechnet. Multipliziert mit der zur Sonne zeigenden Querschnitsfläche $\pi R_{\text{planet}}^2$ ergibt sich die total auftreffende Leistung. Diese wird teils von der Albedo $\alpha$ reflektiert. Zurück bleibt die von der Planetenoberfläche absorbierte Leistung \begin{equation}
...
Aus dem Radius und Oberflächentemperatur $T_{S}$ eines Planeten wird deren Schwarzkörper-Strahlung
\begin{equation}
P_{\text{blackbody}} = 4 \pi R_{\text{planet}}^2 \sigma T_{S}^4
\end{equation}
berechnet. Diese Leistung bleibt jedoch teilweise durch den Treibhauseffekt-Koeffizienten $\beta$ in der Atmosphäre gefangen. Der Anteil der durchdringenden Leistung ist
\begin{equation}
...
Im verwendeten Modell wirkt sich deren Konzentration $H$ linear auf den Teibhauseffekt 
\begin{equation}
\beta  = \xi_8 \cdot H
\end{equation}
aus. Die maximale Wirkung wird mit $\xi_8 \in [0, 1]$ beschränkt und kann vor der Simulation justiert werden.

Die eingesetzte Gleichung der Strahlungsbilanz kann somit mit
\begin{equation}
...
Wolken entstehen, wenn sich feuchte Luft beim aufsteigen abkühlt und kondensiert. Näherungswiese entstehen mehr Wolken je grösser die Luftfeuchtigkeit $H$ ist und je schneller die Temperatur mit der Höhe abnimmt.
...
Die in Tabelle \ref{planeten:xiValues} ersichtlichen Freiheitsgrade $\xi_1$ bis $\xi_6$ wurden empirisch gesetzt um für die Erde stabile Resultate zu erhalten.
