...
\end{figure}
Es soll das Einsetzen von Konvektion in einem dünnen vertikalen Schnitt
durch die Atmosphäre modelliert werden.
...
\]
Die Geschwindigkeit kann mit Hilfe von $\vec{v}=J\nabla \psi$ wieder
...
Differentialgleichungssystem und die Operatoren $D$, $A$ und $N$
...
Vektoren in einem endlichdimensionalen Vektorraum zu ersetzen.
Dann
...
$\sin(ax)\sin(y)$
führen, $\pi/a$ ist die Breite der Zelle.
...
\notag
\end{align}
In \eqref{skript:fluid:trigumf1} haben wir die trigonometrische Identität
...
\quad
\right\}
\label{skript:lorenz:approxeq}
\end{equation}
\subsubsection{Gewöhnliche Differentialgleichungen}
Vergleichen wir in \eqref{skript:lorenz:approxeq}
die Koeffizienten der Funktionen aus der Liste
\eqref{skript:funktionsauswahl},
